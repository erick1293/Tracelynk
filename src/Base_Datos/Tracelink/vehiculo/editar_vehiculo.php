...
if (!empty($data['id']) && !empty($data['marca']) && !empty($data['modelo']) && !empty($data['anio']) && !empty($data['transmision']) && !empty($data['patente']) && !empty($data['kilometrajeinicial']) && !empty($data['kilometrajeactual'])&& !empty($data['idEstado'])) {
    $id = $data['id'];
    $marca = $data['marca'];
    $modelo = $data['modelo'];
    $anio = $data['anio'];
    ...
    $idEstaado = $data['idEstado'];

    // Consulta para actualizar el vehículo usando consultas preparadas
    $stmt = $conn->prepare("UPDATE vehiculo SET marca=?, modelo=?, anio=?, transmision=?, patente=?, kilometrajeinicial=?, kilometrajeactual=? , idEstado=? WHERE id=?");
    $stmt->bind_param("sssssssii", $marca, $modelo, $anio, $transmision, $patente, $kilometrajeinicial, $kilometrajeactual,$idEstaado , $id);
